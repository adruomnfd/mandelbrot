...
{
	if (f01 < 1.0f / 3)
		return {lerp(f01 * 3, 0.0f, 0.0f), lerp(f01 * 3, 0.0f, 0.0f), lerp(f01 * 3, .2f, 0.8f)};
	f01 -= 1.0f / 3;
	if (f01 < 1.0f / 3)
		return {lerp(f01 * 3, 0.0f, 0.4f), lerp(f01 * 3, 0.0f, 0.6f), lerp(f01 * 3, 0.8f, 1.0f)};
	f01 -= 1.0f / 3;
...
public:
	Viewer(Image image) : image(image)
	{
		...
		if (mode->width != image.size.x || mode->height != image.size.y)
			monitor = nullptr;
...
struct uinteger
{
	...
	template <typename T>
	constexpr T &as(int index)
	{
		return *((T *)bits + index);
	}
	template <typename T>
	constexpr const T &as(int index) const
	{
		return *((T *)bits + index);
	}
...
struct highp_float
{
	...
	bool operator>(const int log2) const
	{
		...
		if (exponent > log2 + exponent_offset)
			return true;
...
int main()
{
	...
	*/
	...
	bool any_candicate{};
	std::shared_ptr<complex<LowpType>[]> Xs
	{ new complex<LowpType>[ max_iterations ] };
	std::mutex mutex;
	...
	while (!viewer.should_close())
	{
		...
		static bool isSpaceReleased = true;
